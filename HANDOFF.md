...
## 現在バージョン: v9.9（地図のピンズレを修正。カスタムViewで自前描画）

## ★地図の実装（v9.9で作り直した。ここは触るとき注意）
`IslandMapView.kt`（`View` のサブクラス）が **背景画像とピンを同じ Canvas に自分で描く**。
v9.8 までは ImageView + FrameLayout の絶対配置でピンを載せていたが、
**背景の描画位置とピンの配置基準がズレて、ピンが違う地形の上に出る不具合が発生した**。
レイアウトの解釈に依存しない自前描画に変えたことで、原理的にズレなくなっている。
**ImageView+marginでピンを置く方式に戻さないこと。**

- `onMeasure` は `setMeasuredDimension(mapW, mapH)` を返すだけ
- `onDraw`: `background.setBounds(0,0,mapW,mapH)` → ピンを `x*mapW, y*mapH` に描く
- **ピンはアイコンの「先端」が座標に来る**ように底辺を合わせている（中心合わせではない）
- タップ判定も同じ座標系。横スクロールとの誤爆を防ぐため、
  指が12dp以上動いた場合は選択しない
- 寸法: `mapH = 画面幅`、`mapW = mapH × 画像アスペクト比`（約1.833）。
  可視範囲は幅の約55%なので、初期は「直前に居たエリア」が中央に来るよう
  `OnGlobalLayoutListener` でスクロールする（`post{}` だと間に合わないことがある）
- 画像が無ければ `IslandMap.kt` の描画マップにフォールバック（削除しないこと）

## 座標調整のやり方
`GameData.Area.mapX / mapY`（0.0〜1.0）だけを直す。
検証は Python で `map_island.webp` を `mapW×mapH` にリサイズし、
同じ式でピンを描いて目視するのが確実。
